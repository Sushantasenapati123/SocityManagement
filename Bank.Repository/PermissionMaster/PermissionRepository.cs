...
namespace PathoLab.Repository.PermissionMaster
{
     public class PermissionRepository:RepositoryBase, IPermissionRepository
    {
        ...
        public async Task<List<EmployeeEntity>> DesignationDDL()
        {
            try
            {
                DynamicParameters param = new DynamicParameters();
                param.Add("@action", "GetAllDesg");
                var doc = Connection.Query<EmployeeEntity>("USP_PL_DDL", param, commandType: CommandType.StoredProcedure).ToList();
                return doc;
            }
            catch (Exception ex)
            {

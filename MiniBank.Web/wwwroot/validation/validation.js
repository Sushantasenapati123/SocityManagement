...
function PhonevalidateO(txtPhone, llbError) {
    var phoneText = $("#" + txtPhone).val();

    var filter = /^([0-9]{10})$/;
    if (filter.test(phoneText) || phoneText.length==0) {
        $("#" + llbError).text("");
        return true;
...
function Pinvalidate(Pincode, lblErrorPin) {
    ...
    var filter = /^([0-9]{6})$/;
    if (filter.test(Pincodee) || Pincodee.length == 0) {
        $("#" + lblErrorPin).text("");
        return true;
...
function ValidPan_No(txtPhone, llbError) {
    var phoneText = $("#" + txtPhone).val();
    ...
    if (filter.test(phoneText) || phoneText.length == 0) {
        ...
        $("#" + llbError).text("Invalid Pan Number  Hint-ABCTY1234D");
        return false;
    }
